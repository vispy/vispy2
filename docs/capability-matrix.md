...
| Scalar image/colorbar | DATA `ImageVisual`, color scale, colorbar guide | `Axes.imshow`, `Axes.colorbar` | yes | yes when probed | strict DATA placement and numeric mapping; backend raster/layout | M300 focused unit, offscreen, retained-navigation, paired-runner, post-reboot sizing, and lifecycle checks | temporary current-runtime captures; no release artifact | Datoviz image-texel query remains unadvertised |
